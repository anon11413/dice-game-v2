...
export function FundamentalEstimator() {
  ...
  const F_t = useAnalysisStore((s) => s.F_t);
  const F_tHistory = useAnalysisStore((s) => s.F_tHistory);
  ...
  // Track price history alongside F_t updates
  useEffect(() => {
  ...
  const draw = useCallback(() => {
    ...
    // Compute Y range from both histories
    ...
    // F_t line (blue dashed) — now uses real engine F_t
    ctx.beginPath();
    ...
    // Reveal: actual F_t from dev reveal mode (purple line — should now overlap blue)
    ...
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillText('F_t', pad.left + 20, legendY + 17);

    if (revealEnabled) {
      ctx.fillStyle = '#a855f7';
      ctx.fillRect(pad.left + 4, legendY + 26, 12, 2);
      ctx.fillText('Reveal F_t', pad.left + 20, legendY + 29);
    }
  }, [dims, F_tHistory, revealEnabled, revealHistory]);

  // Gap calculation using real F_t
  const currentGap = price !== 0 && F_t !== 0 ? ((price - F_t) / F_t) * 100 : 0;
  ...
  useEffect(() => {
    dirtyRef.current = true;
  }, [price, F_tHistory, revealHistory]);
        ...
        <div className="flex items-center gap-2">
          ...
          {revealEnabled && revealData && (
            <span className="text-xs text-purple-400">
              Reveal: ${revealData.F_t.toFixed(2)}
            </span>
          )}
        </div>
      </div>
